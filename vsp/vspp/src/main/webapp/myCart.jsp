...
<tbody>
<%
    ResultSet rs = st.executeQuery("select * from product join cart on product.id = cart.product_id where cart.email='"+email+"' and cart.address is NULL");
    while(rs.next()) {
        sno = sno + 1;
%>
<tr>
...
<td>
    <a href="removeFromCart.jsp?id=<%= rs.getString(1) %>">Remove <i class='fas fa-trash-alt'></i></a>
</td>
</tr>
<%
    }
